...
parent_dir		= "../../main_data/hodge_cycles/smith_form/primitive_intersection_matrix/"
result_folder	= "../../main_data/hodge_cycles/elementary_divisors/"
...
result_arxiv = open(result_folder + "hodge_cycles_elementary_divisors_table.tex", 'w')

# Preamble of tex file
result_arxiv.write("\\begin{table}[ht!]\n\\centering\n\\begin{tabular}{ll}\n\\hline\n")
result_arxiv.write("\\multicolumn{1}{ | l | }{$(n, d)$}" + 3*"\t"+ "& \\multicolumn{1}{l | }{Elementary Divisors (Non-Primitive Cycles)}" + 5*"\t"+"\\ \\hline\n")
for s in np.arange(0, len(n_arr)):
	n = n_arr[s]
	for d in np.arange(d_arr_min[s], d_arr_max[s] + 1):
		# Setting filename
		basic_filename	= "primitive_intersection_n_" + str(n) + "_d_" + str(d) + "_inv_factors_diag.txt"
